...
import { mapLocations, mapEras, empires } from '@/data/locations';
...
export function InteractiveMap() {
  ...
  useEffect(() => {
    if (isAutoPlaying) {
      autoPlayRef.current = setInterval(() => {
        setCurrentEraIndex(prev => (prev + 1) % mapEras.length);
      }, 4000);
    } else {
  ...
  // Update visible locations based on era
  useEffect(() => {
    const era = mapEras[currentEraIndex];
    const filtered = mapLocations
      .filter(loc => loc.era === era.id || loc.era === 'Ancient' || loc.era === 'Medieval')
      .map(loc => loc.id);
    ...
    const interval = setInterval(() => {
      if (index < filtered.length) {
        setVisibleLocations(prev => [...prev, filtered[index]]);
        index++;
      } else {
  ...
  const handleLocationClick = (location: MapLocation) => {
    ...
    if (!revealedLocations.includes(location.id)) {
      setRevealedLocations(prev => [...prev, location.id]);
    }
  };
  ...
  const currentEra = mapEras[currentEraIndex];
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <p className="text-[#D7C9A5] text-sm md:text-base">
            Travel across 5,000 years of Indian civilization. Watch kingdoms rise, 
        ...
        <div className="max-w-6xl mx-auto relative">
          <div className="relative bg-[#1C1410] rounded-2xl overflow-hidden border border-[#D4AF37]/10 shadow-2xl shadow-[#D4AF37]/5">
            ...
            {/* Map Area */}
            <div ref={mapRef} className="relative w-full aspect-[4/3] bg-gradient-to-b from-[#1C1410] to-[#0F0F0F] overflow-hidden">
              ...
              >
                <span className="text-sm md:text-base text-[#D4AF37] font-serif">
                  {currentEra.name} • {currentEra.year}
                </span>
              </motion.div>
              ...
              <div className="absolute top-4 right-4 z-20">
                ...
                >
                  ...
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 text-[#D4AF37] font-bold text-xs">N</div>
                  <div className="absolute bottom-0 left-1/2 -translate-x-1/2 text-[#D7C9A5]/30 text-xs">S</div>
                  <div className="absolute top-1/2 left-0 -translate-y-1/2 text-[#D7C9A5]/30 text-xs">W</div>
                  <div className="absolute top-1/2 right-0 -translate-y-1/2 text-[#D7C9A5]/30 text-xs">E</div>
                </motion.div>
              </div>
              ...
              >
                <defs>
                  ...
                  <radialGradient id="fogGrad" cx="50%" cy="50%" r="70%">
                ...
                {/* Mountains - Himalayas */}
                <g opacity="0.2">
                  <path d="M300,80 L350,40 L400,70 L450,30 L500,60 L550,80" stroke="#D4AF37" strokeWidth="2" fill="none" />
                  <path d="M350,90 L370,60 L390,80 L410,50 L430,70 L450,90" stroke="#D4AF37" strokeWidth="1.5" fill="none" />
                  <path d="M250,100 L280,70 L310,90 L340,60 L370,80" stroke="#D4AF37" strokeWidth="1.5" fill="none" />
                  <text x="420" y="55" fill="#D4AF37" opacity="0.3" fontSize="16" fontFamily="serif" textAnchor="middle">
                    HIMALAYAS
                  </text>
                </g>
                ...
                {/* Rivers */}
                <g opacity="0.15">
                  <path d="M400,100 Q420,180 410,280 Q390,380 420,480 Q440,540 430,620" stroke="#D4AF37" strokeWidth="2" fill="none" />
                  <path d="M520,80 Q510,160 530,260 Q550,360 520,450 Q500,510 510,580" stroke="#D4AF37" strokeWidth="1.5" fill="none" />
                  <path d="M300,120 Q310,200 290,300 Q270,400 300,500" stroke="#D4AF37" strokeWidth="1.5" fill="none" />
                </g>

                {/* Empire Borders - Mauryan */}
                <AnimatePresence>
                  {currentEra.id === 'mauryan' && (
                    <motion.g
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                    >
                      <polygon
                        ...
                        strokeWidth="2"
                      />
                      <text x="400" y="380" fill="#D4AF37" opacity="0.4" fontSize="20" fontFamily="serif" textAnchor="middle">
                        Mauryan Empire
                      </text>
                    </motion.g>
                  )}
                ...
                {/* Empire Borders - Maratha */}
                <AnimatePresence>
                  {currentEra.id === 'maratha' && (
                    <motion.g
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                    >
                      <polygon
                        ...
                        strokeWidth="2"
                      />
                      <text x="400" y="480" fill="#C46A00" opacity="0.4" fontSize="20" fontFamily="serif" textAnchor="middle">
                        Maratha Empire
                      </text>
                    </motion.g>
                  )}
                ...
                {mapLocations.map((location) => {
                  const isVisible = visibleLocations.includes(location.id);
                  const isHovered = hoveredLocation === location.id;
                  const isSelected = selectedLocation?.id === location.id;
                  const isRevealed = revealedLocations.includes(location.id);
                  const Icon = getLocationIcon(location.icon);

                  return (
                    <g
                      key={location.id}
                      transform={`translate(${location.x * 10}, ${location.y * 12.5})`}
                      ...
                      {isVisible && (
                        ...
                        >
                          ...
                          <circle
                            ...
                            cy="0"
                            r="18"
                            fill={isHovered || isSelected ? '#D4AF37' : '#2B221C'}
                            stroke="#D4AF37"
                          ...
                          <text
                            ...
                            fill={isHovered || isSelected ? '#D4AF37' : '#D7C9A5'}
            ...
            <div className="p-4 bg-[#1C1410] border-t border-[#D4AF37]/10">
              <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                  <button
                    onClick={() => handleEraChange(Math.max(0, currentEraIndex - 1))}
                  ...
                  <button
                    onClick={() => handleEraChange(Math.min(mapEras.length - 1, currentEraIndex + 1))}
                    className="p-2 bg-[#D4AF37]/10 rounded-lg hover:bg-[#D4AF37]/20 transition-colors border border-[#D4AF37]/20"
                    disabled={currentEraIndex === mapEras.length - 1}
                  >
              ...
              <div className="flex justify-center gap-4 mt-3 text-xs text-[#A09682]">
                <span>🟢 {visibleLocations.length} locations visible</span>
                <span>🔵 {revealedLocations.length} explored</span>
              </div>
            </div>
          </div>
        </div>

        {/* Selected Location Modal - Manuscript Style */}
        <AnimatePresence>
          {selectedLocation && (
            <motion.div
              initial={{ opacity: 0 }}
            ...
            >
              <motion.div
                ...
                exit={{ scale: 0.9, opacity: 0, y: 20, rotateY: 10 }}
                ...
                <div className="relative p-6 md:p-8">
                  ...
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div>
                      ...
                      {selectedLocation.nameHindi && (
                        <p className="text-sm text-[#A09682]">{selectedLocation.nameHindi}</p>
                      )}
                  ...
                  <div className="border-t border-[#D4AF37]/10 pt-4 mt-2">
                    ...
                    <div className="mt-4 flex items-center gap-2 text-sm text-[#D4AF37]">
                      ...
                      <span>Associated with: {selectedLocation.hero}</span>
                    </div>
